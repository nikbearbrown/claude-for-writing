# Chapter 16 — The Annotated Bibliography


## TL;DR

- Proving You Read What You're Citing.
- The chapter moves through What the annotation actually contains, A worked example, Why the evaluation component is always the weakest, The bibliography that never changes its mind, and related ideas.
...
| What it does | Failure mode (with a brief example) | What a strong version looks like (with a brief example) |
| --- | --- | --- |
| Summary, Evaluation, Reflection. Columns: What it does, Failure mode (with a brief example | The pattern becomes easy to misuse or overlook. | Use the chapter example as the concrete test case. |
| What a strong version looks like (with a brief example). | Use the chapter example as the concrete test case. | Use the chapter example as the concrete test case. |
...
![The full worked annotation above shown as an](images/16-annotated-bibliography-gathering-evaluating-and-documenting-sources-fig-01.png)
*Figure 16.1 — The full worked annotation above shown as an*
...
| Item | Meaning |
| --- | --- |
| Publication venue, Author credentials, Date. Each row shows the metadata version (left column) and the evaluation version (right column | A concrete checkpoint for applying the chapter concept. |
| using the chapter's own examples. | Use the chapter example as the concrete test case. |
...
| Category | What it provides that other types cannot | When it is essential vs |
| --- | --- | --- |
| each of the five source categories (peer-reviewed articles, longer works | reports, journalism | magazine, counterargument source, primary source). Columns: Category, What it provides that other types cannot, When it is essential vs. optional, Warning sign if absent. |
...
**What you're building:** the annotated bibliography for your developing research paper, sharpened by structured interrogation of each annotation.

**Tool:** [Claude.ai](https://claude.ai/) chat. A Claude Project recommended for long projects.

**The prompt:**

```
I'm building an annotated bibliography for an argumentative research
paper. The format requires a citation in MLA/APA and then a 100-200
word annotation that summarizes, evaluates, and reflects on each
source.

Below I'm pasting my draft annotations for [N] sources. I have read
each source carefully. The annotations are my first drafts.

Do the following, one source at a time. For each annotation:

1. Does the summary accurately capture what the source actually
   argues, or am I sliding into a related claim? Be specific.

2. Does the evaluation engage with the source's credibility
   seriously? What's missing — author credentials, venue type,
   methodological caveats?

3. Does the reflection place the source in my developing argument
   with enough specificity? Do I name which other sources it
   connects to and how?

4. Is the annotation between 100-200 words? If it's outside the
   range, where can I tighten or expand?

Do not rewrite my annotations. Identify what needs fixing. The
revision is mine.

Here are my draft annotations:
[paste them]
```

**What this produces:** specific revision targets for each annotation. After this, you revise the annotations yourself.
